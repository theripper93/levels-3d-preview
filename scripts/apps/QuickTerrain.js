import { BuildPanelApp } from "./BuildPanelApp.js";

let fileCache = null;

export class QuickTerrain extends BuildPanelApp {
    ...
    async getSources() {
        ...
        for (let target of this.sources) {
            let sourceFiles;
            try {
                sourceFiles = await game.Levels3DPreview.CONFIG.UI.AssetBrowser.getFiles(target, source, "webp");
            } catch (e) {
                try {
                    sourceFiles = await game.Levels3DPreview.CONFIG.UI.AssetBrowser.getFiles(target, "user", "webp");
                } catch (e) {
                    sourceFiles = [];
                }
            }
    ...
    async _onRender(context, options) {
        ...
        if (!game.modules.get("canvas3d-premium")?.active) return this.close();
        html.style.display = "unset";
        
        html.querySelectorAll("button").forEach(button => button.addEventListener("click", (event) => {
            ...
            if (dataAction === "fine-tune") {
                const MatrixEditor = game.Levels3DPreview.CONFIG.UI.MatrixEditor;
                if (MatrixEditor) return new MatrixEditor(this.terrainTile).render({ force: true });
            }
        }));
    }
    ...
    async close(...args) {
        if (this.openAssetBrowser) {
            new game.Levels3DPreview.CONFIG.UI.AssetBrowser().render(true);
        }
