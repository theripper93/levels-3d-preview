...
export class InteractionManager {
    ...
    dragObject() {
        ...
        const isFlying = token && (token?.document?.movementAction === "fly" || (token?.document?.actor?.statuses.some(s=>s.includes("fly")) ?? token?.document?.flags?.["levels-3d-preview"]?.forceFlyingMode));
