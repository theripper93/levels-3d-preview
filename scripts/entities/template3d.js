...
export class Template3D {
    ...
    _getSphereGeometry(){
        const radius = this._origin && this._destination ? this._origin.distanceTo(this._destination) : Ruler3D.unitsToPixels(this.template.data.distance)
    ...
    _getOrigin(A){
        if(A) return A
        return Ruler3D.posCanvasTo3d({x: this.template.data.x, y: this.template.data.y, z: this.template.data.flags?.levels?.elevation ?? 0})
    }
    ...
    _get3DData(){
        if(this.fromData){
            ...
            return {special: _levels.nextTemplateSpecial}
        }
    }
