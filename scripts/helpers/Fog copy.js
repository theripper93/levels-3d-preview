...
export class Fog{
    ...
    async updateTexture(force = false){
        ...
        const size = [this.texWidth, this.texHeight];
        if(!this.shader) return this.fogTexture.dispose();
...
export function injectFoWShaders(THREELIB){
    ...
    THREELIB.ShaderChunk.fog_fragment += `
        ...
        if( vWorldPositionFoW.x >= sceneOrigin.x && vWorldPositionFoW.x <= sceneOrigin.x + sceneDimensions.x && vWorldPositionFoW.z >= sceneOrigin.y && vWorldPositionFoW.z <= sceneOrigin.y + sceneDimensions.y ){
            float sceneX = (vWorldPositionFoW.x - sceneOrigin.x)/sceneDimensions.x;
            float sceneY = (vWorldPositionFoW.z - sceneOrigin.y)/sceneDimensions.y;
